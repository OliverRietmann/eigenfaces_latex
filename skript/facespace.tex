...
\begin{losung}
	\begin{enumerate}[label=(\alph*)]
		...
		\item Die Skizze ist in Abbildung~\ref{fig:meanddiff_simple} dargestellt.
			...
			\captionof{figure}{Durchschnitt $\vec{m}$ der Vektoren $\vec{b}_1,\vec{b}_2,\vec{b}_3$ sowie deren Translationen $\vec{a}_1,\vec{a}_2,\vec{a}_3$ um $-\vec{m}$.}
...
\end{lstlisting}
	...
	\end{minipage}
\end{losung}
Mit dem Durchschnittsgesicht könne wir die sogenannten \textit{Differenzgesichter} $\vec a_1,\ldots,\vec a_K$ bilden.
